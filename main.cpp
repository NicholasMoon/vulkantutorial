...
const std::string TEXTURE_PATH = "textures/gradient.png";
...
struct Vertex {
    ...
    bool operator==(const Vertex& other) const {
        return pos == other.pos && color == other.color && texCoord == other.texCoord && nor == other.nor;
    }
};
...
namespace std {
    template<> struct hash<Vertex> {
        size_t operator()(Vertex const& vertex) const {
            return (((hash<glm::vec3>()(vertex.pos) ^ (hash<glm::vec3>()(vertex.color) << 1)) >> 1) ^ (hash<glm::vec2>()(vertex.texCoord) << 1) ^ (hash<glm::vec3>()(vertex.nor) << 1));
        }
    };
}
...
private:
    ...
    std::vector<Vertex> vertices; // vertices of the tutorial model
    std::vector<uint32_t> indices; // vertex indicies of the tutorial model
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(indices.size()), 1, 0, 0, 0); // vertCt, instCt, firstVert, firstInst
    ...
    void createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
        ...
        imageInfo.samples = numSamples; // for msaa
        imageInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;

        // create vulkan image to store texture in
    ...
    void createTextureImage() {
        ...
        // transition the texture image to VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL (while generating mipmaps)
        //transitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels);
